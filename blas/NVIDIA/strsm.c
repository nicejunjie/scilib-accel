...
#ifdef DBI
#define _STRSM mystrsm
#else 
#define _STRSM strsm_
#endif 

void _STRSM(const char *side, const char *uplo, const char *transa, const char *diag,
            const int *m, const int *n, const float *alpha, const float *A,
            const int *lda, float *B, const int *ldb) {

    enum findex fi = strsm; 
    ...
    if(avgn<scilib_matrix_offload_size)  {
         DEBUG2(fprintf(stderr,"cpu: strsm args: side=%c, uplo=%c, transa=%c, diag=%c, m=%d, n=%d, alpha=%.1e, lda=%d, ldb=%d\n",
         ...
         DEBUG3(fprintf(stderr, "cpu: single strsm timing(s): total= %10.6f\n", t0 ));
...
if(scilib_offload_mode == 1){
    ...
    DEBUG3(fprintf(stderr, "gpu: single strsm timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
