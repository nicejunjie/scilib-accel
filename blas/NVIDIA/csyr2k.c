...
void _CSYR2K(const char *uplo, const char *trans, const int *n, const int *k, const void *alpha, const void* A,
    ...
    if(avgn < scilib_matrix_offload_size) {
        DEBUG2(fprintf(stderr,"cpu: csyr2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=(%.1e, %.1e), \
    ...
    DEBUG2(fprintf(stderr,"gpu: csyr2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=(%.1e, %.1e), \
      lda=%d, ldb=%d, beta=(%.1e, %.1e), ldc=%d\n",
      *uplo, *trans, *n, *k, crealf(*(float complex*)alpha), cimagf(*(float complex*)alpha),
      *lda, *ldb, crealf(*(float complex*)beta), cimagf(*(float complex*)beta), *ldc));
    ...
    if(scilib_offload_mode == 1) {
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasCsyr2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, d_A, *lda, d_B, *ldb, beta, d_C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
        DEBUG1(t1 += mysecond());
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasCsyr2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, A, *lda, B, *ldb, beta, C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
