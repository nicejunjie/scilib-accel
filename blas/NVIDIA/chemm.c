...
#ifdef DBI
#define _CHEMM mychemm
#else 
#define _CHEMM chemm_
#endif 
void _CHEMM(const char *side, const char *uplo, const int *m, const int *n, const float *alpha, const void* A,
            const int *lda, const void* B, const int *ldb, const float *beta, void* C, const int *ldc) {

    enum findex fi = chemm; 
    ...
    if(avgn<scilib_matrix_offload_size)  {
         DEBUG2(fprintf(stderr,"cpu: chemm args: side=%c, uplo=%c, m=%d, n=%d, alpha=(%.1e, %.1e), \
         ...
         DEBUG3(fprintf(stderr, "cpu: single chemm timing(s): total= %10.6f\n", t0 ));
         ...
         DEBUG2(fprintf(stderr,"gpu: chemm args: side=%c, uplo=%c, m=%d, n=%d, alpha=(%.1e, %.1e), \
           lda=%d, ldb=%d, beta=(%.1e, %.1e), ldc=%d\n",
           *side, *uplo, *m, *n, crealf(*(float complex*)alpha), cimagf(*(float complex*)alpha),
           *lda, *ldb,crealf(*(float complex*)beta),cimagf(*(float complex*)beta) , *ldc));
...
//    if( beta_abs > 1.0e-8 )  bug if gemm on a submatrix
    ...
    DEBUG1(t1 -= mysecond());
    CUBLAS_CHECK(cublasChemm(handle, gpu_side, gpu_uplo, *m, *n, alpha, A, *lda, B, *ldb, beta, C, *ldc));
    CUDA_CHECK(cudaDeviceSynchronize());
    DEBUG1(t1 += mysecond());
    ...
    DEBUG3(fprintf(stderr, "gpu: single chemm timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
