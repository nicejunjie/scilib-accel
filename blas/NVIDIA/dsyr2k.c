...
#ifdef DBI
#define _DSYR2K mydsyr2k
#else 
#define _DSYR2K dsyr2k_
#endif 

void _DSYR2K(const char *uplo, const char *trans, const int *n, const int *k, const double *alpha, const double *A,
             const int *lda, const double *B, const int *ldb, const double *beta, double *C, const int *ldc) {

    enum findex fi = dsyr2k; 
    ...
    if(avgn < scilib_matrix_offload_size) {
        DEBUG2(fprintf(stderr,"cpu: dsyr2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=%.1e, \
        ...
        DEBUG3(fprintf(stderr, "cpu: single dsyr2k timing(s): total= %10.6f\n", t0 ));
    ...
    if(scilib_offload_mode == 1) {
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasDsyr2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, d_A, *lda, d_B, *ldb, beta, d_C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
        DEBUG1(t1 += mysecond());
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasDsyr2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, A, *lda, B, *ldb, beta, C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single dsyr2k timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
