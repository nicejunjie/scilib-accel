...
#endif 
void _CSYMM(const char *side, const char *uplo, const int *m, const int *n, const float *alpha, const void* A,
...
//    if( beta_abs > 1.0e-8 )  bug if gemm on a submatrix
    ...
    DEBUG1(t1 -= mysecond());
    CUBLAS_CHECK(cublasCsymm(handle, gpu_side, gpu_uplo, *m, *n, alpha, A, *lda, B, *ldb, beta, C, *ldc));
    CUDA_CHECK(cudaDeviceSynchronize());
    DEBUG1(t1 += mysecond());
