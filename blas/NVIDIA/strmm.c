...
if(scilib_offload_mode == 1){
    ...
    DEBUG1(t1 -= scilib_second());
    CUBLAS_CHECK(cublasStrmm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, d_A, *lda, d_B, *ldb, d_B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    DEBUG1(t1 += scilib_second());
    ...
    DEBUG1(t1 -= scilib_second());
    CUBLAS_CHECK(cublasStrmm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, A, *lda, B, *ldb, B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
