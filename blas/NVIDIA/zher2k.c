...
void _ZHER2K(const char *uplo, const char *trans, const int *n, const int *k, const void *alpha, const void* A,
    ...
    CUBLAS_CHECK(cublasSetStream(scilib_cublas_handle, current_cuda_stream));
        CUBLAS_CHECK(cublasZher2k(scilib_cublas_handle, gpu_uplo, gpu_trans, *n, *k, alpha, d_A, *lda, d_B, *ldb, beta, d_C, *ldc));
        CUDA_CHECK(cudaStreamSynchronize(current_cuda_stream));
        DEBUG1(t1 += scilib_second());
        ...
        DEBUG1(t1 -= scilib_second());
    CUBLAS_CHECK(cublasSetStream(scilib_cublas_handle, current_cuda_stream));
        CUBLAS_CHECK(cublasZher2k(scilib_cublas_handle, gpu_uplo, gpu_trans, *n, *k, alpha, A, *lda, B, *ldb, beta, C, *ldc));
        CUDA_CHECK(cudaStreamSynchronize(current_cuda_stream));
