...
//         printf("%s %.1f\n", "sgemm on cpu", avgn);
    ...
    fprintf(stderr,"gpu: sgemm args: transa=%c, transb=%c, m=%d, n=%d, k=%d, alpha=%.1f, lda=%d, ldb=%d, beta=%.1f, ldc=%d\n",
...
//    if( beta_abs > 1.0e-8 )  bug if gemm on a submatrix
    ...
    status = cublasSgemm(handle, transA, transB, *m, *n, *k, alpha, d_A, *lda, d_B, *ldb, beta, d_C, *ldc);
    CUDA_CHECK(cudaDeviceSynchronize());
