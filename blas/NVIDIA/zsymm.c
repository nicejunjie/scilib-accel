...
#endif 
void _ZSYMM(const char *side, const char *uplo, const int *m, const int *n, const void *alpha, const void* A,
    ...
    if(avgn<scilib_matrix_offload_size)  {
         DEBUG2(fprintf(stderr,"cpu: zsymm args: side=%c, uplo=%c, m=%d, n=%d, alpha=(%.1e, %.1e), lda=%d, ldb=%d, beta=(%.1e, %.1e), ldc=%d\n",
