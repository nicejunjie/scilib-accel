...
void _ZHEMM(const char *side, const char *uplo, const int *m, const int *n, const void *alpha, const void* A,
    ...
    if(avgn<scilib_matrix_offload_size)  {
         DEBUG3(fprintf(stderr,"cpu: zhemm args: side=%c, uplo=%c, m=%d, n=%d, alpha=(%.1e, %.1e), lda=%d, ldb=%d, beta=(%.1e, %.1e), ldc=%d\n",
         ...
         DEBUG3(fprintf(stderr, "cpu: single zhemm timing(s): total= %10.6f\n", t0 ));
...
//    if( beta_abs > 1.0e-8 )  bug if gemm on a submatrix
    ...
    DEBUG3(fprintf(stderr, "gpu: single zhemm timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
