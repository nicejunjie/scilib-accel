...
#ifdef DBI
#define _CHERK mycherk
#else 
#define _CHERK cherk_
#endif 

void _CHERK(const char *uplo, const char *trans, const int *n, const int *k, const float *alpha, const void* A,
            const int *lda, const float *beta, void* C, const int *ldc) {

    enum findex fi = cherk; 
    ...
    if(avgn < scilib_matrix_offload_size) {
        ...
        DEBUG3(fprintf(stderr, "cpu: single cherk timing(s): total= %10.6f\n", t0 ));
    ...
    if(scilib_offload_mode == 1) {
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasCherk(handle, gpu_uplo, gpu_trans, *n, *k, alpha, d_A, *lda, beta, d_C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
        DEBUG1(t1 += mysecond());
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasCherk(handle, gpu_uplo, gpu_trans, *n, *k, alpha, A, *lda, beta, C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single cherk timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
