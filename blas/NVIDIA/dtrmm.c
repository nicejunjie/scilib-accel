...
#ifdef DBI
#define _DTRMM mydtrmm
#else 
#define _DTRMM dtrmm_
#endif 

void _DTRMM(const char *side, const char *uplo, const char *transa, const char *diag,
            const int *m, const int *n, const double *alpha, const double *A,
            const int *lda, double *B, const int *ldb) {

    enum findex fi = dtrmm; 
    ...
    if(avgn<scilib_matrix_offload_size)  {
         ...
         DEBUG3(fprintf(stderr, "cpu: single dtrmm timing(s): total= %10.6f\n", t0 ));
...
if(scilib_offload_mode == 1){
    ...
    DEBUG1(t1 -= mysecond());
    CUBLAS_CHECK(cublasDtrmm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, d_A, *lda, d_B, *ldb, d_B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    DEBUG1(t1 += mysecond());
    ...
    DEBUG1(t1 -= mysecond());
    CUBLAS_CHECK(cublasDtrmm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, A, *lda, B, *ldb, B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single dtrmm timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
