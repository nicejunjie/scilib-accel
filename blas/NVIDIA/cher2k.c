...
#ifdef DBI
#define _CHER2K mycher2k
#else 
#define _CHER2K cher2k_
#endif 

void _CHER2K(const char *uplo, const char *trans, const int *n, const int *k, const void *alpha, const void* A,
             const int *lda, const void* B, const int *ldb, const void *beta, void* C, const int *ldc) {

    enum findex fi = cher2k; 
    ...
    if(avgn < scilib_matrix_offload_size) {
        DEBUG2(fprintf(stderr,"cpu: cher2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=(%.1e, %.1e), \
        ...
        DEBUG3(fprintf(stderr, "cpu: single cher2k timing(s): total= %10.6f\n", t0 ));
    ...
    DEBUG2(fprintf(stderr,"gpu: cher2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=(%.1e, %.1e), \
      lda=%d, ldb=%d, beta=(%.1e, %.1e), ldc=%d\n",
      *uplo, *trans, *n, *k, crealf(*(float complex*)alpha), cimagf(*(float complex*)alpha),
      *lda, *ldb, crealf(*(float complex*)beta), cimagf(*(float complex*)beta), *ldc));
    ...
    if(scilib_offload_mode == 1) {
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasCher2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, d_A, *lda, d_B, *ldb, beta, d_C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
        DEBUG1(t1 += mysecond());
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasCher2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, A, *lda, B, *ldb, beta, C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single cher2k timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
