...
#ifdef DBI
#define _SSYR2K myssyr2k
#else 
#define _SSYR2K ssyr2k_
#endif 

void _SSYR2K(const char *uplo, const char *trans, const int *n, const int *k, const float *alpha, const float *A,
             const int *lda, const float *B, const int *ldb, const float *beta, float *C, const int *ldc) {

    enum findex fi = ssyr2k; 
    ...
    if(avgn < scilib_matrix_offload_size) {
        DEBUG2(fprintf(stderr,"cpu: ssyr2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=%.1e, \
        ...
        DEBUG3(fprintf(stderr, "cpu: single ssyr2k timing(s): total= %10.6f\n", t0 ));
    ...
    if(scilib_offload_mode == 1) {
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasSsyr2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, d_A, *lda, d_B, *ldb, beta, d_C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
        DEBUG1(t1 += mysecond());
        ...
        DEBUG1(t1 -= mysecond());
        CUBLAS_CHECK(cublasSsyr2k(handle, gpu_uplo, gpu_trans, *n, *k, alpha, A, *lda, B, *ldb, beta, C, *ldc));
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single ssyr2k timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
