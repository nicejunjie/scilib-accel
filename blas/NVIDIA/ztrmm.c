...
#ifdef DBI
#define _ZTRMM myztrmm
#else 
#define _ZTRMM ztrmm_
#endif 

void _ZTRMM(const char *side, const char *uplo, const char *transa, const char *diag,
            const int *m, const int *n, const void *alpha, const void *A,
            const int *lda, void *B, const int *ldb) {

    enum findex fi = ztrmm; 
    ...
    if(avgn<scilib_matrix_offload_size)  {
         DEBUG2(fprintf(stderr,"cpu: ztrmm args: side=%c, uplo=%c, transa=%c, diag=%c, m=%d, n=%d, alpha=(%.1e, %.1e), lda=%d, ldb=%d\n",
         ...
         DEBUG3(fprintf(stderr, "cpu: single ztrmm timing(s): total= %10.6f\n", t0 ));
...
if(scilib_offload_mode == 1){
    ...
    DEBUG1(t1 -= scilib_second());
    CUBLAS_CHECK(cublasZtrmm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, d_A, *lda, d_B, *ldb, d_B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    DEBUG1(t1 += scilib_second());
    ...
    DEBUG1(t1 -= scilib_second());
    CUBLAS_CHECK(cublasZtrmm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, A, *lda, B, *ldb, B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single ztrmm timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
