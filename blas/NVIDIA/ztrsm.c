...
#ifdef DBI
#define _ZTRSM myztrsm
#else 
#define _ZTRSM ztrsm_
#endif 

void _ZTRSM(const char *side, const char *uplo, const char *transa, const char *diag,
            const int *m, const int *n, const void *alpha, const void *A,
            const int *lda, void *B, const int *ldb) {

    enum findex fi = ztrsm; 
    ...
    if(avgn<scilib_matrix_offload_size)  {
         ...
         DEBUG3(fprintf(stderr, "cpu: single ztrsm timing(s): total= %10.6f\n", t0 ));
...
if(scilib_offload_mode == 1){
    ...
    DEBUG1(t1 -= mysecond());
    CUBLAS_CHECK(cublasZtrsm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, d_A, *lda, d_B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    DEBUG1(t1 += mysecond());
    ...
    DEBUG1(t1 -= mysecond());
    CUBLAS_CHECK(cublasZtrsm(handle, gpu_side, gpu_uplo, gpu_transa, gpu_diag, *m, *n, alpha, A, *lda, B, *ldb));
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    DEBUG3(fprintf(stderr, "gpu: single ztrsm timing(s): total= %10.6f, compute= %10.6f, other= %10.6f\n", t0, t1, t0-t1));
