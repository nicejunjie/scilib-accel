...
#ifdef DBI
#define _ZSYR2K myzsyr2k
#else 
#define _ZSYR2K zsyr2k_
#endif 

void _ZSYR2K(const char *uplo, const char *trans, const int *n, const int *k, const void *alpha, const void* A,
             const int *lda, const void* B, const int *ldb, const void *beta, void* C, const int *ldc) {

    enum findex fi = zsyr2k; 
    ...
    if(avgn < scilib_matrix_offload_size) {
        DEBUG2(fprintf(stderr,"cpu: zsyr2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=(%.1e, %.1e), \
    ...
    DEBUG2(fprintf(stderr,"gpu: zsyr2k args: uplo=%c, trans=%c, n=%d, k=%d, alpha=(%.1e, %.1e), \
      lda=%d, ldb=%d, beta=(%.1e, %.1e), ldc=%d\n",
      *uplo, *trans, *n, *k, creal(*(double complex*)alpha), cimag(*(double complex*)alpha),
      *lda, *ldb, creal(*(double complex*)beta), cimag(*(double complex*)beta), *ldc));
